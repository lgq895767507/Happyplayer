...
void NEFFmpeg::_prepare() {
    ...
    for (int i = 0; i < formatContext->nb_streams; ++i) {
        ...
        if (codecParameters->codec_type == AVMEDIA_TYPE_AUDIO) {
            ...
            //视频
            //获取帧率
            AVRational av_frame = stream->avg_frame_rate;
            double fps = av_q2d(av_frame);
...
void NEFFmpeg::seekTo(int playProgress) {
    ...
    if (!audioChannel && !videoChannel) {
        if (javaCallHelper) {
            javaCallHelper->onError(THREAD_CHILD, ERROR_CODE_PROGRESS_VIDEO_OR_AUDIO_NULL);
        }
        return;
    }
    ...
    if (!formatContext) {
        if (javaCallHelper) {
            javaCallHelper->onError(THREAD_CHILD, ERROR_CODE_PROGRESS_FORMAT_CONTEXT_NULL);
        }
        return;
    }
    ...
    if (ret < 0) {
        if (javaCallHelper) {
            javaCallHelper->onError(THREAD_CHILD, ERROR_CODE_AV_SEEK_FRAME);
        }
        return;
    }
